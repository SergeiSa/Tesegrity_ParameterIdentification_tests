function ParameterEstimation_EvaluateRegressor_experimental(RegressorStructure, ExpData)
...
Count = ExpData.Count; mm = m*3; 
compundM = zeros(mm*Count, length(value_mu)+length(value_rho)); 
compundLHS = zeros(mm*Count, 1); 

for i = 1:Count
r = ParameterEstimation_load_experimental_data(robot, ExpData, i);
